...
"""Frosty Actions API – structured planning/validation for Canvas.
...
Routes
------
- POST /api/v1/frosty/suggest_v2  → structured actions, costs, hints
- POST /api/v1/frosty/validate    → dry validation of actions against a blueprint
- POST /api/v1/frosty/simulate    → apply patches in-memory and return the result
- POST /api/v1/frosty/run_plan    → lightweight run plan (graph or selection)
...
class CopilotAction(BaseModel):
    """Single structured Frosty action.
