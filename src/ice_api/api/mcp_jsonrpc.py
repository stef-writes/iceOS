...
async def mcp_jsonrpc_handler(request: Request) -> Union[MCPResponse, Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"Processing MCP method: {mcp_request.method}", extra={
        ...
        try:
            ...
            logger.info(f"Successfully processed {mcp_request.method}", extra={
                "method": mcp_request.method,
                "request_id": request_id
            })
            ...
            logger.error(f"Internal error in {mcp_request.method}: {e}\n{traceback.format_exc()}")
            return {
                "jsonrpc": "2.0",
                "id": request_id,
...
async def handle_initialize(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if client_version != "2024-11-05":
        logger.warning(f"Client protocol version {client_version} may not be fully compatible")
    ...
    logger.info("MCP client connected", extra={
...
async def handle_tools_call(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(f"Executing MCP tool: {tool_name}", extra={
        "tool_name": tool_name,
        "arguments": arguments
    })
    ...
    if ":" not in tool_name:
        raise ValueError(f"Invalid tool name format: {tool_name}. Expected format: 'type:name'")
    ...
    try:
        ...
        logger.info(f"Started execution for {tool_name}", extra={
        ...
        logger.info(f"Completed execution for {tool_name}", extra={
            "run_id": run_ack.run_id,
        ...
        logger.error(f"Error executing {tool_name}: {e}", extra={
            "tool_name": tool_name,
            "error": str(e)
        })
        raise
...
async def validate_tool_exists(tool_type: str, name: str) -> None:
    ...
    if tool_type == "tool":
        ...
        available_agents = [agent_name for agent_name, agent_path in global_agent_registry.available_agents()]
        if name not in available_agents:
            raise ValueError(f"Agent '{name}' not found")
    elif tool_type == "workflow":
        workflow_names = [wf_name for node_type, wf_name in registry.list_nodes(NodeType.WORKFLOW)]
...
async def handle_prompts_list() -> Dict[str, Any]:
    ...
    prompts.extend([
        MCPPrompt(
            ...
            arguments=[
                {"name": "sector", "description": "Investment sector to analyze", "required": True},
        ...
        MCPPrompt(
            ...
            arguments=[
                {"name": "document_sources", "description": "Document sources to process", "required": True},
                {"name": "chunk_strategy", "description": "Document chunking strategy", "required": False}
            ]
        ).dict(),
        MCPPrompt(
            ...
            arguments=[
                {"name": "platform", "description": "Marketplace platform", "required": True},
                {"name": "inventory_source", "description": "Inventory data source", "required": True}
            ]
        ).dict()
    ])
...
def get_documentation(doc_name: str) -> str:
    ...
    # For now, return a placeholder
    return f"# {doc_name.title()} Documentation\n\nDocumentation content for {doc_name}..." 