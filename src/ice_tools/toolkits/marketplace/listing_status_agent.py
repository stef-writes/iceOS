...
class ListingStatusAgent(MemoryAgent):
    ...
    async def _execute_with_memory(
        self, enhanced_inputs: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        outcome_analysis = await self._analyze_conversation_outcome(
            conversation_result, status_history
        )
        ...
        if update_plan["strategy"] == "update_to_sold":
            ...
            result = await self._execute_unavailable_update(
                listing_id, conversation_result
            )
            action = "update_status"
        ...
        await self._learn_from_status_update(
            outcome_analysis, update_plan, result.get("success", False)
        )
    ...
    async def _analyze_conversation_outcome(
        self, conversation_result: Dict[str, Any], status_history: List[Any]
    ) -> Dict[str, Any]:
...
Provide your analysis:"""
        ...
        if error:
            ...
            return {
                ...
                "item_unavailable": conversation_result.get("action")
                == "item_unavailable",
                "status_update_needed": conversation_result.get("action")
                in ["sale_completed", "item_unavailable"],
                "update_type": (
                    "sold"
                    if conversation_result.get("action") == "sale_completed"
                    else "none"
                ),
                "confidence": 0.5,
    ...
    async def _plan_status_update(
        self, analysis: Dict[str, Any], listing_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        # Item no longer available
            ...
            analysis.get("item_unavailable")
            and analysis.get("update_type") == "unavailable"
        ):
            return {
    ...
    async def _execute_sale_update(
        self, listing_id: str, conversation_result: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_unavailable_update(
        self, listing_id: str, conversation_result: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _learn_from_status_update(
        self, analysis: Dict[str, Any], plan: Dict[str, Any], success: bool
    ) -> None:
