...
class MarketplaceConversationAgent(MemoryAgent):
    ...
    async def _execute_with_memory(
        ...
        inquiry_analysis = await self._analyze_inquiry(
            latest_message, conversation_text
        )

        # STEP 2: PLAN response strategy
        response_plan = await self._plan_response_strategy(
            inquiry_analysis, listing_context
        )
        ...
        await self._learn_from_interaction(
            inquiry_analysis, response_plan, requires_human
        )
...
Provide your analysis:"""
            ...
            return {
                ...
                "complexity": (
                    ...
                    if any(
                        ...
                        for word in ["available", "have", "stock"]
                    )
                    else "complex"
                ),
    ...
    async def _plan_response_strategy(
        ...
        if analysis.get("complexity") == "simple" and analysis.get("inquiry_type") in [
            "availability",
            "general",
        ]:
            return {
        ...
        elif analysis.get("inquiry_type") in [
            "pricing",
            "delivery",
            "condition",
            "negotiation",
    ...
    async def _learn_from_interaction(
        self, analysis: Dict[str, Any], plan: Dict[str, Any], required_human: bool
    ) -> None:
    ...
    def _build_conversation_context(
        self, messages: List[Dict[str, str]], memory_history: List[Any]
    ) -> str:
    ...
    async def _generate_simple_response(
        self, message: str, conversation_context: str, listing_context: Dict[str, Any]
    ) -> str:
...
- Location: {listing_location}""".format(
            **listing_context
        )
