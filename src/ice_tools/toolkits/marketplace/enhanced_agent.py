...
class EnhancedMarketplaceAgent(MemoryAgent):
    ...
    async def _execute_with_memory(
        ...
        product_knowledge = await self._load_product_knowledge(
            listing_context.get("listing_item", "")
        )
...
Provide detailed analysis:"""
    ...
    async def _plan_tool_usage(
        ...
        available_tools = (
            [tool.get("name", "") for tool in self.tools] if self.tools else []
        )
    ...
    async def _execute_tools(
        self, tool_plan: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
...
Provide a helpful, personalized response:"""
    ...
    async def _load_customer_preferences(self, customer_id: str) -> Dict[str, Any]:
        ...
        try:
            preferences = await self.memory.search(
                f"customer_preferences:{customer_id}"
            )
    ...
    async def _load_product_knowledge(self, product_name: str) -> Dict[str, Any]:
        ...
        try:
            knowledge = await self.memory.search(
                f"product_knowledge:{product_name.lower()}"
            )
    ...
    def _get_tool_parameters(
        ...
        if tool_name == "price_calculator":
            ...
            return {"product_id": context.get("product_id", "unknown")}
        return {}
    ...
    async def _simulate_tool_execution(
        ...
        if tool_name == "price_calculator":
            ...
            return {"status": "success", "in_stock": True, "quantity": 3}
