"""Workflow Builder engine (formerly Chain Builder) – relocated from ice_cli.chain_builder.engine.

Stateless helper to drive the interactive *Workflow Builder* flow.  The public
API is stable: `WorkflowBuilder`, `WorkflowDraft`, `Question`.  Legacy names
`BuilderEngine` and `ChainDraft` remain as deprecated shims for backward
compatibility.
"""
...
class BuilderEngine:  # noqa: D401 – stateless helper
    ...
    def next_question(draft: ChainDraft) -> Optional[Question]:
        ...
        if draft.current_step == 0:
            ...
            if draft.nodes and draft.nodes[-1]["type"] == "llm":
                return Question(key="model", prompt="Model (llm only)")
            draft.current_step += 1
            return BuilderEngine.next_question(draft)
        elif draft.current_step == 3:
            if draft.nodes and draft.nodes[-1]["type"] == "llm":
                return Question(
                    key="skills", prompt="Allowed skills (comma-separated, blank=all)"
                )
            draft.current_step += 1
            return BuilderEngine.next_question(draft)
    ...
    def render_chain(draft: ChainDraft) -> str:
        ...
        for idx, node in enumerate(draft.nodes):
            ...
            if node["type"] == "llm":
                ...
                node_lines.append(
                    f"    LLMOperatorConfig(id=\"{node_id}\", type=\"llm\", name=\"{node['name']}\", model=\"{node.get('model', get_default_model_id())}\", prompt=\"# TODO\", llm_config={{'provider': 'openai'}}, dependencies={deps_str}{tools_str}{extra_str}),"
                )
            else:
                node_lines.append(
                    f"    SkillNodeConfig(id=\"{node_id}\", type=\"skill\", name=\"{node['name']}\", skill_name=\"echo\", skill_args={{}}, dependencies={deps_str}{extra_str}),"
                )
