...
class AtomicWorkflowPrinciples:
    ...
    def simplify_node_choice(task_description: str, suggested_type: str) -> str:
        ...
        if any(
            word in task_lower
            for word in ["read", "write", "fetch", "save", "call", "execute"]
        ):
            if "csv" in task_lower or "json" in task_lower or "api" in task_lower:
                return "tool"

        # Rule 2: If it's just text generation, use 'llm' not 'agent'
        if suggested_type == "agent" and not any(
            word in task_lower for word in ["tool", "memory", "state"]
        ):
            return "llm"
        ...
        if suggested_type == "swarm":
            ...
            if not any(
                word in task_lower for word in ["vote", "consensus", "majority"]
            ):
        ...
        if any(
            word in task_lower
            for word in ["calculate", "transform", "process", "convert"]
        ):
            return "code"
    ...
    def validate_workflow_practicality(nodes: List[Dict[str, Any]]) -> List[str]:
        ...
        if total_nodes > 0:
            complexity_score = (
                sum(
                    AtomicWorkflowPrinciples.COMPLEXITY_RANKING.get(
                        node.get("type", "unknown"), 10
                    )
                    for node in nodes
                )
                / total_nodes
    ...
    def generate_clarifying_questions(nodes: List[Dict[str, Any]]) -> List[str]:
        ...
        for i, node in enumerate(nodes):
            ...
            if any(
                word in description.lower() for word in ["process", "handle", "manage"]
            ):
                questions.append(
