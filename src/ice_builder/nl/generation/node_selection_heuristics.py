...
CAPABILITY_HEURISTICS: Dict[str, Callable[[str], Optional[Type[BaseNodeConfig]]]] = {
    ...
    "requires_memory": lambda desc: (
        AgentNodeConfig if "remember" in desc.lower() else None
    ),
    "requires_iteration": lambda desc: (
        LoopNodeConfig
        if any(word in desc.lower() for word in ["repeat", "iterate", "loop"])
        else None
    ),
    "requires_parallelism": lambda desc: (
        ParallelNodeConfig
        if any(
            word in desc.lower()
            for word in ["parallel", "concurrent", "simultaneously"]
        )
        else None
    ),
    "requires_consensus": lambda desc: (
        SwarmNodeConfig
        if any(word in desc.lower() for word in ["vote", "consensus", "committee"])
        else None
    ),
    "requires_approval": lambda desc: (
        HumanNodeConfig
        if any(word in desc.lower() for word in ["approve", "review", "human"])
        else None
    ),
    "generates_code": lambda desc: (
        CodeNodeConfig
        if any(word in desc.lower() for word in ["code", "script", "program"])
        else None
    ),
    "monitors_metrics": lambda desc: (
        MonitorNodeConfig
        if any(word in desc.lower() for word in ["monitor", "watch", "alert"])
        else None
    ),
}
...
def select_node_type(
    ...
    # 3. Default to LLM node for general text tasks
