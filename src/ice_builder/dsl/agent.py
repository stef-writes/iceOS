...
class AgentBuilder:
    ...
    def with_llm(
        self, model: str = "gpt-4", provider: ModelProvider = ModelProvider.OPENAI
    ) -> "AgentBuilder":
    ...
    def with_memory(
        self, memory_config: Optional[Dict[str, Any]] = None
    ) -> "AgentBuilder":
    ...
    def build(self) -> Dict[str, Any]:
        ...
        if "system_prompt" not in self.config["config"]:
            raise ValueError(
                "Agent requires system prompt. Call with_system_prompt() first."
            )
