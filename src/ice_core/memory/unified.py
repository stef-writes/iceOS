...
class UnifiedMemory:
    ...
    def __init__(self, config: Optional[UnifiedMemoryConfig] = None):
        ...
        if config.enable_working:
            working_config = config.working_config or MemoryConfig(
                backend="memory", enable_vector_search=config.enable_vector_search
            )
        ...
        if config.enable_episodic:
            episodic_config = config.episodic_config or MemoryConfig(
                backend=config.backend, enable_vector_search=config.enable_vector_search
            )
    ...
    async def retrieve(
        self, key: str, memory_type: Optional[str] = None
    ...
    async def delete(self, key: str, memory_type: Optional[str] = None) -> bool:
        """Delete from appropriate memory.
        ...
        return await self._memories[memory_type].delete(key)
    ...
    async def remember_fact(
        self, fact: str, metadata: Optional[Dict[str, Any]] = None
    ) -> None:
    ...
    async def get_domain_analytics(self) -> Dict[str, Any]:
        ...
        """
        analytics = {"domains": self.config.domains, "domain_usage": {}}
