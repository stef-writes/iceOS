...
class ProceduralMemory(BaseMemory):
    ...
    async def _load_default_procedures(self) -> None:
        ...
        await self.store(
            ...
            content={
                ...
                "steps": [
                    {"action": "acknowledge_offer", "template": "I appreciate your offer!"},
                    {"action": "evaluate_offer", "condition": "offer >= price * 0.8"},
                    {"action": "counter_offer", "formula": "price * 0.9", "template": "I could do ${counter} for you."},
                    {"action": "create_urgency", "template": "I have other interested buyers, so let me know soon!"}
                ],
                "applicable_when": {
                    "offer_range": [0.7, 0.9],
        ...
        await self.store(
            ...
            content={
                ...
                "steps": [
                    {"action": "confirm_interest", "template": "Great choice! You're going to love this {item}."},
                    {"action": "arrange_pickup", "template": "When would work best for pickup? I'm available {availability}."},
    ...
    async def clear(self, pattern: Optional[str] = None) -> int:
        ...
        if pattern:
            keys_to_delete = [k for k in self._procedures.keys() if k.startswith(pattern)]
        else:
    ...
    async def record_execution(
        ...
        if "learned_adjustments" in outcome:
            await self._apply_learned_adjustments(procedure_key, outcome["learned_adjustments"])
    ...
    async def _apply_learned_adjustments(
        ...
        entry.metadata["modification_count"] = entry.metadata.get("modification_count", 0) + 1
    ...
    async def get_best_procedures(
        ...
        experienced_procedures = [
            ...
            if p.metadata.get("usage_count", 0) >= 5
        ]
        
        return experienced_procedures or procedures[:3]  # Return top 3 if none are experienced
    ...
    async def export_successful_procedures(
        ...
        for key, entry in self._procedures.items():
            ...
            if (metadata.get("success_rate", 0) >= min_success_rate and
                metadata.get("usage_count", 0) >= min_usage):
