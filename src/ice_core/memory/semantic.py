...
class SemanticMemory(BaseMemory):
    ...
    def __init__(self, config: MemoryConfig):
        ...
        self._relationships: Dict[str, Dict[str, List[Tuple[str, str]]]] = defaultdict(
    ...
    def get_relationships_by_type(
        ...
        return {
            k: [(target, float(strength)) for target, strength in v]
    ...
    async def store(
        ...
        for rel in entry.metadata.get("relationships", []):
            if isinstance(rel, dict) and "target" in rel and "type" in rel:
                rel_type = rel["type"]
                self._relationships[rel_type][key].append(
                    (rel["target"], rel.get("strength", 1.0))
                )
    ...
    def validate_embedding(self, vector: List[float]) -> None:  # noqa: D401
        ...
        if len(vector) != self._embedding_dim:
            ...
            raise DimensionMismatchError(
                expected=self._embedding_dim, actual=len(vector)
            )
    ...
    async def delete(self, key: str) -> bool:
        ...
        for rel in entry.metadata.get("relationships", []):
            if isinstance(rel, dict) and "type" in rel:
                rel_type = rel["type"]
                if (
                    rel_type in self._relationships
                    and key in self._relationships[rel_type]
                ):
    ...
    async def clear(self, pattern: Optional[str] = None) -> int:
        ...
        if pattern:
            keys_to_delete = [
                k for k in self._facts_store.keys() if k.startswith(pattern)
            ]
        else:
    ...
    async def find_related(
        self, entity: str, relationship_type: Optional[str] = None, limit: int = 10
    ) -> List[MemoryEntry]:
                ...
                if relationship_type:
                    ...
                    if isinstance(relationships_data, list):
                        ...
                        for rel_data in relationships_data:
                            if (
                                isinstance(rel_data, (list, tuple))
                                and len(rel_data) >= 2
                            ):
                                relationships.append(
                                    (str(rel_data[0]), float(rel_data[1]))
                                )
    ...
    async def get_knowledge_graph(
        ...
        while queue:
            ...
            # Add node
            graph["nodes"][entity] = {"type": "entity", "depth": depth, "facts": []}
        ...
        if "sale_price" in interaction_data and "item_condition" in interaction_data:
            ...
            await self.store(
                ...
                metadata={
                    ...
                    "entities": [
                        interaction_data["item_type"],
                        interaction_data["item_condition"],
                    ],
                    "domain": "marketplace",
                    "confidence": 0.8,
                    "source": "learned",
    ...
    async def get_domain_knowledge(
        self, domain: str, fact_type: Optional[str] = None
    ) -> List[MemoryEntry]:
    ...
    async def merge_facts(self, keys: List[str], merged_key: str) -> bool:
