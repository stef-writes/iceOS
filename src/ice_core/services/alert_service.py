...
class AlertNotificationProtocol(Protocol):
    ...
    async def send_alert(
        self, message: str, severity: AlertSeverity, metadata: Dict[str, Any]
    ) -> bool:
    ...
    async def send_alert(
        ...
        try:
            ...
            getattr(logger, log_level)(
                f"🚨 ALERT: {message}", severity=severity.value, metadata=metadata
            )
            return True
        except Exception as e:
...
class AlertService:
    ...
    def register_handler(
        self, channel: AlertChannel, handler: AlertNotificationProtocol
    ) -> None:
        """Register an alert handler for a specific channel."""
        self._handlers[channel] = handler
        logger.info(f"Registered alert handler for {channel.value}")

    def configure_email_alerts(self, smtp_config: Dict[str, Any]) -> None:
    ...
    async def send_monitoring_alert(
        ...
        message = (
            f"Monitoring condition triggered on node '{node_id}': {metric_expression}"
        )
