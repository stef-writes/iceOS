...
async def validate_workflow_definition(
    ...
    # Check for naming conflicts using workflow factories
    if (
        definition.name,
        f"{definition.name}",
    ) in registry.available_workflow_factories():
...
async def validate_component(
    ...
    if not re.match(r"^[a-zA-Z][a-zA-Z0-9_-]*$", definition.name):
        return ComponentValidationResult(
            valid=False,
            errors=[
                "Component name must start with letter and contain only letters, numbers, underscore, or dash"
            ],
            component_type=definition.type,
            registered=False,
            registry_name=None,
