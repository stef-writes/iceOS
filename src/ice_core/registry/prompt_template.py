...
class PromptTemplateRegistry:
    ...
    def register(self, name: str, template: MessageTemplate) -> None:
        ...
        Raises:
            RegistryError: If a template with this name already exists
        """
        if name in self._templates:
            from ice_core.exceptions import RegistryError

            raise RegistryError(f"Template '{name}' already registered")
    ...
    def get(self, name: str) -> MessageTemplate:
        ...
        Raises:
            RegistryError: If template not found
        """
        if name not in self._templates:
            from ice_core.exceptions import RegistryError

            raise RegistryError(f"Template '{name}' not found")
...
def register_prompt_template(
    name: str,
) -> Callable[[Callable[[], MessageTemplate]], Callable[[], MessageTemplate]]:
