...
global_agent_registry.register_agent(${class_name}.name, "${module_path}:${class_name}")
''')
...
class ${class_name}AgentTool(ToolBase):
    ...
    async def _execute_impl(self, *, messages: List[Dict[str, str]]) -> Dict[str, Any]:  # noqa: D401
        AgentCls = global_agent_registry.get_agent_class("${agent_name}")
        agent = AgentCls()
        reply = await agent.run(messages=messages)
        return {"reply": reply}

_instance = ${class_name}AgentTool()
...
_instance = ${class_name}LLMOperator()
