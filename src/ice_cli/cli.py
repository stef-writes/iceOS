...
def init_project(
    ...
    if project_path.exists() and not force:
        rprint(f"[red]Error:[/] Directory '{name}' already exists. Use --force to overwrite.")
        raise typer.Exit(1)
...
def create_resource(
    ...
    if resource_type == "chain":
        ...
        if target_path.exists() and not force:
            ...
            raise typer.Exit(1)
        content = f'"""Example ScriptChain generated by `ice create chain`."""\n\nfrom ice_orchestrator import ScriptChain\n\nclass {name.capitalize()}Chain(ScriptChain):\n    """Describe what the chain does."""\n'
        ...
        if target_path.exists() and not force:
            rprint(f"[red]Error:[/] File {target_path} already exists. Use --force to overwrite.")
            raise typer.Exit(1)
        content = f'from ice_sdk.tools.base import BaseTool\n\nclass {name}(BaseTool):\n    name = "{name.lower()}"\n    description = "Describe what the tool does"\n    async def run(self, ctx, **kwargs):\n        return {{}}\n'
        target_path.write_text(content)
        rprint(f"[green]✔[/] Created {target_path}")
        ...
        if target_path.exists() and not force:
            rprint(f"[red]Error:[/] File {target_path} already exists. Use --force to overwrite.")
            raise typer.Exit(1)
        content = f'id: {name}_ai\ntype: ai\nname: {name}\nmodel: gpt-3.5-turbo\nprompt: |\n  # TODO: write prompt here\nllm_config:\n  provider: openai\n  temperature: 0.7\n  max_tokens: 256\ndependencies: []\n'
        target_path.write_text(content)
        rprint(f"[green]✔[/] Created {target_path}")
    else:
        rprint(f"[red]Error:[/] Unknown resource type '{resource_type}'. Use: chain, tool, node")
        raise typer.Exit(1)
...
from ice_sdk.copilot.cli import (
    copilot_app,
)
