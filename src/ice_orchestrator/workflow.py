...
# Canonical node executor implementation
...
class Workflow(BaseWorkflow):  # type: ignore[misc]  # mypy cannot resolve BaseScriptChain across namespace package boundary
    ...
    def __init__(
        ...
        *,
        ...
        # Initialize event handler with Redis stream sink for production-grade delivery
        # Avoid importing API layer here; sinks should be injected by API during startup
        ...
        self._graph_analyzer = GraphAnalyzer(
            self.graph.graph
        )  # Use the NetworkX graph from DependencyGraph
    ...
    async def _handle_recursive_flows(
        ...
        for source_id, recursive_context in recursive_executions:
            await self._execute_recursive_node(
                source_id, recursive_context, accumulated_results
            )
        ...
        try:
            ...
            await self._event_handler.emit(
                WorkflowCompleted(
                    workflow_id=self.chain_id,
                    duration_seconds=(
                        self._execution_state.end_time
                        - self._execution_state.start_time
                    ).total_seconds(),
    ...
    def _get_workflow_debug_info(self) -> Dict[str, Any]:
        ...
        # Node type breakdown
        node_breakdown: Dict[str, int] = cast(
            Dict[str, int], debug_info["node_breakdown"]
        )
        ...
        # Execution order
        execution_order: List[Dict[str, Any]] = cast(
            List[Dict[str, Any]], debug_info["execution_order"]
        )
        ...
        if self._execution_state:
            debug_info["execution_summary"] = (
                self._execution_state.get_execution_summary()
            )
            ...
            for dep_id in node.dependencies:
                ...
                if dep_node and hasattr(dep_node, "output_schema"):
                    ...
                    if not self._schemas_compatible(
                        dep_node.output_schema, node.input_schema
                    ):
    ...
    async def execute_incremental(
        ...
        """
        if from_checkpoint:
            self._execution_state = WorkflowExecutionState.from_checkpoint(
                from_checkpoint
            )
        else:
            self._execution_state = WorkflowExecutionState(
                workflow_id=self.chain_id, workflow_name=self.name or "unnamed"
            )
    ...
    def estimate_cost(self, context_size: int = 1000) -> Any:
        ...
        return self._cost_estimator.estimate_workflow_cost(
            list(self.nodes.values()), context_size
        )
    ...
    def suggest_next_nodes(self, after_node_id: str) -> List[Dict[str, Any]]:
        ...
        if hasattr(node, "output_schema") and node.output_schema:
            if (
                isinstance(node.output_schema, dict) and "data" in node.output_schema
            ) or "array" in str(node.output_schema):
                # Consider parallelization if we're not already branching heavily
                if (
                    out_degree < 2
                    and graph_metrics["parallel_opportunities"]
                    < graph_metrics["total_nodes"] * 0.4
                ):
                    suggestions.append(
                        {
                            "type": "parallel",
            ...
            if (
                isinstance(node.output_schema, dict) and "text" in node.output_schema
            ) or "string" in str(node.output_schema):
                suggestions.append(
                    {
                        "type": "tool",
    ...
    async def execute_node(
        ...
        try:
            ...
            EventBus.publish(
                "node.completed",
                {
                    ...
                    "success": result.success,
                    "output_preview": (
                        str(result.output)[:250] if result.output else None
                    ),
                    "error": result.error,
        ...
        self.graph.update_execution_stats(
            ...
            success=result.success,
            error=(
                str(result.error) if hasattr(result, "error") and result.error else None
            ),
        )
                        ...
                        "graph_metrics": {
                            ...
                            "completion_percentage": len(
                                [
                                    ...
                                    if self.graph.graph.nodes[n].get("execution_state")
                                    == "completed"
                                ]
                            )
                            / max(self.graph.graph.number_of_nodes(), 1)
                            * 100,
                        },
                    },
                )
    ...
    def as_workflow_node(
        ...
        *,
        ...
        >>> # Register a workflow factory first
        >>> from ice_core.unified_registry import register_workflow_factory
        >>> register_workflow_factory("checkout_workflow", "my_pkg.workflows:create_checkout_workflow")
        >>> # Then reference it in a node
