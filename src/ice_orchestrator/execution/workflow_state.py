...
class WorkflowExecutionState:
    ...
    def record_node_complete(self, node_id: str, result: NodeExecutionResult) -> None:
        ...
        raw_type = (
            getattr(result.metadata, "node_type", None)
    ...
    def should_checkpoint(self) -> bool:
        ...
        time_since_checkpoint = (
            datetime.utcnow() - self.last_checkpoint
        ).total_seconds()
                        ...
                        "metadata": (
                            {
                                ...
                                "start_time": (
                                    res.metadata.start_time.isoformat()
                                    if res.metadata.start_time
                                    else None
                                ),
                                "end_time": (
                                    res.metadata.end_time.isoformat()
                                    if res.metadata.end_time
                                    else None
                                ),
    ...
    @classmethod
    def from_checkpoint(
        cls, checkpoint_data: Dict[str, Any]
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_nodes": len(self.completed_nodes)
            + len(self.failed_nodes)
            + len(self.skipped_nodes),
