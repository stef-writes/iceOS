...
class GraphAnalyzer:
    ...
    def get_metrics(self) -> GraphMetrics:
        ...
        try:
            max_depth = (
                max(
                    nx.shortest_path_length(self.graph, source).values()
                    for source in self.get_root_nodes()
                )
                or 0
            )
        except ValueError:
        ...
        parallel_opportunities = sum(
            len(nodes) for nodes in levels.values() if len(nodes) > 1
        )
            ...
            for i, node_id in enumerate(nodes):
                ...
                layout_hints[node_id] = {
                    ...
                    "graph_metrics": {
                        ...
                        "betweenness": nx.betweenness_centrality(self.graph).get(
                            node_id, 0
                        ),
                ...
                if node_config:
                    layout_hints[node_id]["style"] = self._get_graph_aware_style(
                        node_config, layout_hints[node_id]
                    )
    ...
    def _compute_complexity_score(self) -> float:
        ...
        density: float = (
            float(n_edges / (n_nodes * (n_nodes - 1))) if n_nodes > 1 else 0.0
        )
    ...
    def _get_graph_aware_style(
        self, node_config: NodeConfig, layout_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Get styling based on graph position and properties."""

        style: Dict[str, Any] = {"opacity": 1.0, "border_width": 1, "highlight": False}
