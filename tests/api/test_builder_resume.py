...
async def test_builder_export_resume_roundtrip():
    async with AsyncClient(app=app, base_url="http://test") as client:
        ...
        resp = await client.post(
            "/api/v1/builder/start", json={"total_nodes": 1, "name": "resume_demo"}
        )
        ...
        next_q_resp = await client.get(
            "/api/v1/builder/next", params={"draft_id": draft_id}
        )
        if next_q_resp.json():
        ...
        export_resp = await client.get(
            "/api/v1/builder/export", params={"draft_id": draft_id}
        )
        ...
        resume_resp = await client.post(
            "/api/v1/builder/resume", json={"draft": draft_payload}
        )
        ...
        q_resp = await client.get(
            "/api/v1/builder/next", params={"draft_id": new_draft_id}
        )
