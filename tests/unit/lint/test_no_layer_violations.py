...
def test_example_imports():
    ...
    try:
        ...
        assert not hasattr(marketing_chain, '_internal'), "Examples must not access private APIs"
    except ImportError:
                ...
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        if alias.name.startswith('app.'):
                            violations.append(f"Direct import from app.* in {file_path}: {alias.name}")
                elif isinstance(node, ast.ImportFrom):
                    if node.module and node.module.startswith('app.'):
                        violations.append(f"Direct import from app.* in {file_path}: {node.module}")
...
def test_service_layer_usage():
    ...
    from ice_sdk.services import ServiceLocator, ChainService
