...
from ice_orchestrator.execution.executors.builtin import (
    recursive_node_executor as recursive_executor,
)
...
class TestRecursiveNodeConfig:
    ...
    def test_recursive_config_validation_no_package_or_workflow(self):
        ...
        with pytest.raises(
            ValueError, match="must specify either agent_package or workflow_ref"
        ):
            RecursiveNodeConfig(
                id="test_recursive", type="recursive", recursive_sources=["node1"]
            )
    ...
    def test_recursive_config_validation_both_package_and_workflow(self):
        ...
        with pytest.raises(
            ValueError, match="cannot specify both agent_package and workflow_ref"
        ):
            RecursiveNodeConfig(
                id="test_recursive",
                type="recursive",
    ...
    def test_recursive_config_validation_no_sources(self):
        ...
        with pytest.raises(
            ValueError, match="must specify at least one recursive_source"
        ):
            RecursiveNodeConfig(
                id="test_recursive",
                type="recursive",
...
class TestRecursiveExecutor:
    ...
    @pytest.mark.asyncio
    async def test_recursive_executor_max_iterations(
        self, sample_recursive_config, mock_workflow
    ):
    ...
    @pytest.mark.asyncio
    async def test_recursive_executor_convergence_condition_met(
        self, sample_recursive_config, mock_workflow
    ):
    ...
    @pytest.mark.asyncio
    async def test_recursive_executor_continues_when_no_convergence(
        self, sample_recursive_config, mock_workflow
    ):
        ...
        with patch(
            "ice_orchestrator.execution.executors.builtin.agent_node_executor"
        ) as mock_agent_exec:
            mock_result = NodeExecutionResult(
                ...
                metadata=NodeMetadata(
                    node_id="negotiation_loop", node_type="agent", name="test_agent"
                ),
            )
            mock_agent_exec.return_value = mock_result

            result = await recursive_executor(
                mock_workflow, sample_recursive_config, ctx
            )
...
class TestWorkflowRecursiveExecution:
    ...
    def test_cycle_detection_blocks_invalid_cycles(self):
        ...
        buyer = AgentNodeConfig(id="buyer", type="agent", package="test.buyer")
        seller = AgentNodeConfig(
            id="seller", type="agent", package="test.seller", dependencies=["buyer"]
        )
