...
async def test_csv_reader_happy(tmp_path):
    """CSVReaderTool should parse rows and return JSON-serialisable output."""

    csv_file = tmp_path / "sample.csv"
    csv_file.write_text("item,qty\napple,3\nbanana,5\n", encoding="utf-8")

    tool = CSVReaderTool()
    out = await tool.execute(file_path=str(csv_file), delimiter=",")
...
async def test_csv_reader_missing_file(tmp_path):
    """Missing file should raise *ToolExecutionError*."""

    missing_path = tmp_path / "does_not_exist.csv"
    tool = CSVReaderTool()

    with pytest.raises(ToolExecutionError):
        await tool.execute(file_path=str(missing_path), delimiter=",") 