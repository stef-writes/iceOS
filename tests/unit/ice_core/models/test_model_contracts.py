...
from ice_core.models import AgentNodeConfig, LLMNodeConfig, ModelProvider

# Import the LLMConfig that LLMNodeConfig actually uses
...
class TestLLMConfigContract:
    ...
    def test_llm_config_fields(self):
        ...
        config = LLMConfig(
            provider=ModelProvider.OPENAI, api_key="sk-test", timeout=30, max_retries=3
        )
...
class TestLLMNodeConfigContract:
    """Documents the actual structure of LLMNodeConfig."""

    def test_llm_node_config_fields(self):
        """LLMNodeConfig has these exact fields."""
...
class TestAgentNodeConfigContract:
    ...
    def test_agent_node_config_fields(self):
        ...
        config = AgentNodeConfig(
            id="agent1",
            type="agent",
            package="default_agent",  # NOT agent_ref!
            tools=[],
            max_iterations=10,
        )
