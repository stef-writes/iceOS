...
async def _await_done(c: httpx.AsyncClient, exec_id: str, headers: Dict[str, str]) -> Dict[str, Any]:
    for _ in range(60):
...
async def test_api_llm_to_search_to_llm() -> None:
    ...
    async with httpx.AsyncClient(base_url="http://testserver", transport=transport) as c:
        ...
        r = await c.post("/api/v1/blueprints/", headers={**headers, "X-Version-Lock": "__new__"}, json=bp)
        assert r.status_code in (200, 201), r.text
        bp_id = r.json()["id"]
        r = await c.post(
            "/api/v1/executions/",
            headers={**headers, "Content-Type": "application/json"},
            json={"payload": {"blueprint_id": bp_id, "inputs": {"topic": "renewable energy"}}},
        )
